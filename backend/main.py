...
async def get_all_pool_tables(db: _orm.Session = _fastapi.Depends(_services.get_db)):
...
async def get_pool_table(
    pool_table_id: int = _fastapi.Path(
        ..., title="The ID of the pool table to retrieve", ge=1
    ),
    db: _orm.Session = _fastapi.Depends(_services.get_db),
):
    pool_table = await _services._pool_table_selector(
        pool_table_id=pool_table_id, db=db
    )
...
async def update_pool_table(
    ...
    pool_table_db = await _services.update_pool_table(
        pool_table_id, updated_pool_table, db
    )
